...
export default function AllUsers() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        // Fetch SmartSoft data
        const usersResponse = await axios.get(
          `${API_URL}/api/admin/all_smart`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        setError("Failed to fetch SmartSoft data. Please try again later.");
        router.push("/login");
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-2xl font-bold mb-6">SmartSoft Data</h1>
