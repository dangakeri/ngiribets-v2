...
function DividerLine({ label }) {
  return (
    <div className="flex items-center my-2 w-full">
      ...
      {label && (
        <span className="mx-3 text-red-500 border border-red-600 px-3 py-1 rounded-full text-xs sm:text-sm md:text-base font-bold">
          {label}
        </span>
      )}
      <div className="flex-grow border-t border-gray-600"></div>
...
function FooterContent() {
  return (
    <footer className="bg-bgBody text-textGrey pt-8 pb-6 mt-4">
      <div className="container mx-auto px-4 flex flex-col gap-6">
        ...
        {/* Policies */}
        <div className="flex flex-row sm:flex-row flex-wrap gap-2 sm:gap-4 mt-4 text-xs sm:text-sm justify-center text-center">
          ...
          <Link
            href="https://www.responsiblegambling.or.ke/"
            target="_blank"
        ...
        <p className="text-xs sm:text-sm text-center text-muted-foreground">
          &copy; {new Date().getFullYear()} Ngiribets. All rights reserved.
        </p>
      </div>
    </footer>
  );
